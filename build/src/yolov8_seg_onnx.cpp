...
bool Yolov8SegOnnx::ReadModel(const std::string &modelPath, bool isCuda,
  ...
  try {
    ...
    if (isCuda && (cuda_available == available_providers.end())) {
      ...
      std::cout << "************* Infer model on GPU! *************"
                // << "\n====>this is ReadModel(" << modelPath << ", " << isCuda
                // << ", " << cudaID << ", " << warmUp
                // << ") in yolov8_seg_onnx.cpp, line23."
                << std::endl;
    ...
    if (_outputNodesNum != 2) {
      cout << "This model has " << _outputNodesNum
           << "output, which is not a segmentation model.Please check your "
              "model name or path!"
           << endl;
      return false;
    }
    ...
    _output_name1 =
        std::move(_OrtSession->GetOutputNameAllocated(1, allocator));
#endif
...
int Yolov8SegOnnx::Preprocessing(const std::vector<cv::Mat> &srcImgs,
...
bool Yolov8SegOnnx::OnnxDetect(cv::Mat &srcImg,
...
bool Yolov8SegOnnx::OnnxBatchDetect(
    std::vector<cv::Mat> &srcImgs,
    std::vector<std::vector<OutputSeg>> &output) {
...
//   cout << "*****START postprocess*****" << endl;
  ...
  vector<int> mask_protos_shape = {1, (int)_outputMaskTensorShape[1],
                                   (int)_outputMaskTensorShape[2],
                                   (int)_outputMaskTensorShape[3]};
  ...
  for (int img_index = 0; img_index < srcImgs.size(); ++img_index) {
    Mat output0 =
        Mat(Size((int)_outputTensorShape[2], (int)_outputTensorShape[1]),
            CV_32F, all_data)
    ...
    std::vector<vector<float>>
        picked_proposals; // output0[:,:, 5 + _className.size():net_width]===>
                          // for mask
    ...
    Mat mask_protos = Mat(mask_protos_shape, CV_32F,
                          output_tensors[1].GetTensorMutableData<float>() +
                              img_index * mask_protos_length);
    for (int i = 0; i < temp_mask_proposals.size(); ++i) {
      GetMask2(Mat(temp_mask_proposals[i]).t(), mask_protos, temp_output[i],
               mask_params);
    }
